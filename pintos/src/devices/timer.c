...
{
  /*Chech the time of threads's ticks_blocked*/
  ...
  if (thread_mlfqs)
  {
    ...
    received "recently." On each timer tick, the running 
    thread's recent_cpu is incremented by 1. Once per second, 
    every thread's recent_cpu is updated this way.*/
  ...
  recalculated once per second as follows.*/
    ...
    /*Every thread has a nice value between -20 and 20 directly
     under its control. Each thread also has a priority, 
     between 0 (PRI_MIN) through 63 (PRI_MAX), which is recalculated 
